...
include("../conexion/conexion.php");
    ...
    if ($data) {            
            ...
            CROSS APPLY(SELECT * FROM dbo.fnSplit(t.Proveedor_Id,',') where t.ObjID in ($amarillo)) Proveedor_Id   
            )t1 
            on t1.ID_Proveedor=p.objID 
            where Severidad_ID=2											--Tipo de severidad (ejemplo amarillo)
            )
            t2
            on t2.ID_Proveedor=contrato.Proveedor_ID
            where contrato.Empresa_Interna_ID in ($amarillo) 
