...
while ($filaProveedor = sqlsrv_fetch_array($ejecutar, SQLSRV_FETCH_NUMERIC)){	
			...
			$cantidadPro=0;
			if (utf8_decode($filaProveedor[8]) != "") {

				$sql2="SELECT  count(*) 
				FROM MFProyectoServicioEspecializado t 
				CROSS APPLY(SELECT * FROM dbo.fnSplit(t.Proveedor_Id,',')  WHERE 
				t.Estatus_Proyecto = 'En Proceso' AND 
				t.Proveedor_ID = '".utf8_decode($filaProveedor[8])."') Proveedor_Id";

				// $sql2="SELECT count(*)  FROM MFProyectoServicioEspecializado WHERE 
				// Estatus_Proyecto ='En Progreso' 
				// AND dbo.fnSplit( Proveedor_ID = '".utf8_decode($filaProveedor[8])."',',') ";				

				$ejecutar2=sqlsrv_query($con, $sql2);
				while ($Proyectos = sqlsrv_fetch_array($ejecutar2, SQLSRV_FETCH_ASSOC)) {
					$cantidadPro = $Proyectos['Proveedor_ID'];						
				}
			} else {
				$cantidadPro=0;
			}							
