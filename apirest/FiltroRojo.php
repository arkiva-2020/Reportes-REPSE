...
include("../conexion/conexion.php");
    ...
    if ($data) {            
            ...
            CROSS APPLY(SELECT * FROM dbo.fnSplit(t.Proveedor_Id,',') where t.ObjID in ($rojo)) Proveedor_Id   
            )t1 
            on t1.ID_Proveedor=p.objID 
            where Severidad_ID=4											--Tipo de severidad (ejemplo rojo)
            )
            t2
            on t2.ID_Proveedor=contrato.Proveedor_ID
            where contrato.Empresa_Interna_ID in ($rojo) 
